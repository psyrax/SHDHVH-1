<div class="span-24 mainb">
	<div class="span-10 first prepend-2 append-1">
		<div id="box">
			<div id="form">
				<h2>Submit</h2>
				<p id="box_desc">Submit a website to be revamped, remodeled and redesigned!</p>
				<form id="submit" method="post" action="" method="post" class="validate">
					<p class="instructions">Website</p><input type="text" name="website" placeholder="http://..." required/>
					<p class="instructions">Needs</p> <textarea name="needs" placeholder="design, ux, html, css, javascript" cols='20' rows='5' required></textarea>
					<p class="instructions">Submitter</p><input type="text" name="submiter" placeholder="Name" required/>
					<p class="instructions">Description</p><textarea name="description" placeholder="This website needs help with... because..." cols='20' rows='5' required></textarea>
					<input type="submit" value="Submit" name="submit" class="button" />
				</form>
			</div>
		</div>
	</div>
	<div class="span-10 last">
		<div id="box">
			<h2>Adopt</h2>
			<div class="details">
				<div class="information">
					<p><a href="http://" >Website</a></p>
					<p>Needs:</p>
					<p>Submitter:</p>
				</div>
				<div class="thumb">
					<img src="/images/thumb.png" width="75" height="75">
				</div>
				<button type="button">Browse</button>
			</div>
		</div>
	</div>
</div>
<div class="reset"></div>
<div class="span-21 push-2 mainc">
	<div class="mainctitle">
		<p>Succesfully Revamped</p>
	</div>
	<div class="done">
		<p><a href="http://" >Website</a></p>
		<img src="/images/thumb.png" width="75" height="75">
	</div>
</div>